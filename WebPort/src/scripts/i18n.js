...
function flipElement(el, newHTML, delay) {
  return new Promise(resolve => {
    gsap.to(el, {
      rotateX: 90, opacity: 0, duration: 0.22, delay,
      ease: 'power2.in', transformOrigin: '50% 50%',
      onComplete: () => {
        el.innerHTML = newHTML;
        gsap.fromTo(el,
          { rotateX: -90, opacity: 0 },
          { rotateX: 0, opacity: 1, duration: 0.28, ease: 'back.out(1.4)', onComplete: resolve }
        );
      }
    });
  });
...
function switchLang(lang) {
  ...
  const promises = targets.map((el, i) => flipElement(el, t[el.dataset.i18n], i * 0.07));
