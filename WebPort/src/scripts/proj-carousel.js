...
import { drawProjectCanvas } from './canvas.js'
...
if (!stage) throw new Error('projStage not found')
...
function positionCards() {
  ...
  cards.forEach((card, i) => {
    const ca    = i * STEP + a
    const x     = RX * Math.sin(ca)
    const y     = RY * Math.cos(ca) * -0.4
    const depth = Math.cos(ca)
    ...
    const t  = (depth + 1) / 2
    const sc = 0.68 + 0.32 * t
    const op = 0.30 + 0.70 * t
