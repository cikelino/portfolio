// 3D Tilt effect su project e skill cards — GSAP quickTo per performance

const TILT_MAX = 8;    // gradi massimi di rotazione (±8°)
const PERSP    = 800;  // prospettiva in px

document.querySelectorAll('.project-card, .skill-card').forEach(card => {
  // Setup 3D
  gsap.set(card, { transformPerspective: PERSP, transformStyle: 'preserve-3d' });

  // Shine overlay — effetto luce speculare
  ...
  card.addEventListener('mousemove', e => {
    ...
    // Normalizza posizione mouse nella card: -1 (sx/top) → +1 (dx/bottom)
    const nx = ((e.clientX - rect.left)  / rect.width)  * 2 - 1;
    const ny = ((e.clientY - rect.top)   / rect.height) * 2 - 1;

    setRY( nx * TILT_MAX);
    setRX(-ny * TILT_MAX); // inverti Y: mouse in alto → top verso viewer

    // Shine segue il mouse dentro la card
